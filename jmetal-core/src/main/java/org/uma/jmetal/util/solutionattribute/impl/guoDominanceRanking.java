...
public class guoDominanceRanking <S extends Solution<?>>
  ...
  public Double value = 0.0000001;

  /**
   * Constructor
   */
  public guoDominanceRanking(Double value) {
    rankedSubPopulations = new ArrayList<>();
    this.value = value;
  }

  public guoDominanceRanking(Object id) {
    super(id) ;
    rankedSubPopulations = new ArrayList<>();
  }
  ...
  public Ranking<S> computeRanking(List<S> solutionSet) {
    ...
    for (int p = 0; p < (population.size() - 1); p++) {
      ...
      for (int q = p + 1; q < population.size(); q++) {
    	  DoubleSolution tmp = (DoubleSolution) solutionSet.get(q);
    	  for(int tm = 0;tm<tmp.getNumberOfVariables();tm++){
    		  tmp.setVariableValue(tm,(tmp.getVariableValue(tm)+value));
    	  }
        flagDominate =
            CONSTRAINT_VIOLATION_COMPARATOR.compare(solutionSet.get(p), tmp);
        if (flagDominate == 0) {
          flagDominate = DOMINANCE_COMPARATOR.compare(solutionSet.get(p), tmp);
        }
        if (flagDominate == -1) {
